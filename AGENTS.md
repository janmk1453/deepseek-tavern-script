...
- `DeepSeek_Statistic_V2.21.json` — Packaged JSON for manual import; copy & rename from previous version on release. `content` must stay in sync with JS.
...
3. **Sync JSON** — extract the IIFE into the versioned JSON's `content` field:
   ```bash
    node -e "var f=require('fs');var js=f.readFileSync('DeepSeek使用预测.js','utf8');var i=js.indexOf('(function()');var json=JSON.parse(f.readFileSync('DeepSeek_Statistic_V2.21.json','utf8'));json.content=js.substring(i);f.writeFileSync('DeepSeek_Statistic_V2.21.json',JSON.stringify(json,null,2)+'\n');console.log('synced')"
    ```
4. Validate: `node --check DeepSeek使用预测.js` and `node -e "new Function(JSON.parse(require('fs').readFileSync('DeepSeek_Statistic_V2.21.json','utf8')).content);console.log('valid')"`
...
```bash
# 1. 创建 release
gh release create v2.21 --title "release 2.21" --notes "<changelog>"

# 2. 上传资产（--clobber 允许覆盖已有文件）
gh release upload v2.21 --clobber "DeepSeek_Statistic_V2.21.json" "DeepSeek_Statistic_auto_update.json" "DeepSeek_Statistic_auto_update_jsDelivr_cdn.json"
...
| 文件 | 说明 | 推荐 |
|------|------|------|
| \DeepSeek_Statistic_V2.21.json\ | 完整版脚本，手动导入 | 需要特定版本时使用 |
...
### 手动更新使用方法
1. 下载 \DeepSeek_Statistic_V2.21.json\
2. 在 SillyTavern 中的酒馆助手导入该文件
3. 每次更新需重新导入
```

其中 2.21 替换为实际版本号，各更新内容章节按实际情况填写，无内容的章节可省略。
