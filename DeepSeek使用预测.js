...
function init() { 
  ...
  function loadSavedData() { 
    try { 
      ...
      if (state.settings.newPricingDate === undefined) state.settings.newPricingDate = new Date('2026-07-15T00:00:00+08:00').getTime(); if (state.settings.customModels === undefined) state.settings.customModels = []; if (state.settings.peakHours === undefined) state.settings.peakHours = JSON.parse(JSON.stringify(DEFAULT_PEAK_HOURS));
  ...
  function normalizeImportData(raw) {
    ...
    if (settings.newPricingDate === undefined) settings.newPricingDate = new Date('2026-07-15T00:00:00+08:00').getTime(); if (settings.customModels === undefined) settings.customModels = []; if (settings.peakHours === undefined) settings.peakHours = JSON.parse(JSON.stringify(DEFAULT_PEAK_HOURS));
    d.settings = settings;
...
function saveCustomModelRow(model, prices, isBuiltin) {
  ...
  recalcAllCosts();
  refreshUI();
  if (state.chartPanelOpen) refreshChartModelRadios();
}
...
function buildChartModelRadiosHtml() {
  var models = getModelList();
  var colors = ['#93c5fd', '#a5b4fc', '#fca5a5', '#fcd34d', '#6ee7b7', '#c4b5fd', '#fdba74', '#7dd3fc', '#bef264', '#f9a8d4'];
  var html = '';
  for (var i = 0; i < models.length; i++) {
    var m = models[i];
    html += '<label style="font-size:11px;padding:3px 8px;border-radius:4px;background:#1e293b;color:' + colors[i % colors.length] + ';cursor:pointer;border:1px solid transparent;font-family:\'Microsoft YaHei\',\'微软雅黑\',sans-serif"><input type="radio" name="ds-chart-model" value="' + m + '" style="display:none"' + (i === 0 ? ' checked' : '') + '><span>' + shortModelV2(m) + '</span></label>';
  }
  return html;
}
function refreshChartModelRadios() {
  var p = window.parent || window;
  var doc = p.document;
  var group = doc.getElementById('ds-chart-model-group');
  if (!group) return;
  var models = getModelList();
  if (models.indexOf(state.chartModel) === -1) state.chartModel = models[0] || '';
  group.innerHTML = buildChartModelRadiosHtml();
  var target = group.querySelector('input[value="' + state.chartModel + '"]');
  if (target) target.checked = true;
  var radios = group.querySelectorAll('input[name="ds-chart-model"]');
  radios.forEach(function(r) {
    r.onchange = function() {
      if (this.checked) { state.chartModel = this.value; updateChartModelSelection(); renderCharts(); }
    };
  });
...
function toggleCharts() {
  ...
  if (!ov || !pn) { createChartUI(); state.chartPanelOpen = true; refreshChartModelRadios(); ov = p.document.getElementById('ds-chart-overlay'); pn = p.document.getElementById('ds-chart-panel'); if (ov) ov.style.display = 'block'; if (pn) pn.classList.add('ds-open'); renderCharts(); return; }
  if (state.chartPanelOpen) {
...
function renderCharts() {
  ...
  var model = getModelList().indexOf(state.chartModel) !== -1 ? state.chartModel : (getModelList()[0] || 'deepseek-v4-flash');
