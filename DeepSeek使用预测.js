...
function createChartUI() {
  ...
  panel.innerHTML = '<div style="padding:12px 16px;background:#080d14;border-bottom:1px solid #374151;display:flex;justify-content:space-between;align-items:center;flex-shrink:0"><div style="display:flex;align-items:center;gap:8px"><span style="font-size:16px;font-weight:600;color:#f3f4f6;font-family:system-ui,-apple-system,sans-serif">统计详情</span><div id="ds-chart-model-group" style="display:flex;gap:4px;margin-left:8px">' +
  ...
  '</div></div><div style="display:flex;align-items:center;gap:8px"><button id="ds-btn-chart-reset" style="padding:4px 8px;border:1px solid #374151;border-radius:4px;background:#0e1520;color:#9ca3af;font-size:11px;cursor:pointer;font-family:system-ui,-apple-system,sans-serif">重置缩放</button><div id="ds-btn-chart-help" style="width:24px;height:24px;background:#1e293b;border-radius:50%;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:12px;color:#9ca3af;font-weight:700;font-family:system-ui,-apple-system,sans-serif">?</div><div id="ds-btn-close-charts" style="width:28px;height:28px;background:#374151;border-radius:6px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:14px;color:#9ca3af">✕</div></div></div><div id="ds-chart-body" style="flex:1;overflow-y:auto;padding:12px 16px 24px;background:#060a10;font-family:system-ui,-apple-system,sans-serif">' +
    ...
    '<div id="ds-chart-container" style="display:none"><div id="ds-model-summary" style="margin-bottom:16px;padding:12px;background:#0e1520;border:1px solid #374151;border-radius:8px;overflow-x:auto"><div style="font-size:12px;color:#9ca3af;font-weight:500;margin-bottom:8px">模型汇总</div><table style="width:100%;border-collapse:collapse;font-size:11px;color:#e5e7eb"><thead><tr style="color:#6b7280;border-bottom:1px solid #374151"><th style="text-align:left;padding:6px 8px;white-space:nowrap">模型</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">调用次数</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">输入(命中)</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">输入(未命中)</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">输出</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">总Token</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">总成本</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">平均成本</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">平均耗时</th><th style="text-align:right;padding:6px 8px;white-space:nowrap">平均速率</th></tr></thead><tbody id="ds-summary-tbody"><tr><td colspan="10" style="text-align:center;padding:20px;color:#6b7280">加载中...</td></tr></tbody></table></div><div style="margin-bottom:20px"><div style="font-size:12px;color:#9ca3af;font-weight:500;margin-bottom:8px;display:flex;align-items:center;gap:4px;flex-wrap:wrap">图1 · Token 趋势<span id="ds-toggle-token" style="font-size:10px;padding:2px 7px;border-radius:3px;background:#374151;color:#9ca3af;cursor:pointer">轮次</span><select id="ds-month-token" style="font-size:11px;padding:2px 6px;border-radius:4px;background:#1e293b;color:#93c5fd;border:1px solid #6366f1;display:none;cursor:pointer;font-family:system-ui,-apple-system,sans-serif"></select></div><div style="position:relative;height:40vh;min-height:280px"><canvas id="ds-chart-token"></canvas></div><div id="ds-slider-token-track" class="ds-chart-slider-track" style="margin:0"><div id="ds-slider-token-thumb" class="ds-chart-slider-thumb"><div class="ds-slider-handle ds-slider-handle-left"></div><div class="ds-slider-handle ds-slider-handle-right"></div></div></div><div id="ds-slider-token-label" class="ds-chart-slider-label"></div></div>' +
      ...
      '<div style="margin-bottom:20px"><div style="font-size:12px;color:#9ca3af;font-weight:500;margin-bottom:8px;display:flex;align-items:center;gap:4px;flex-wrap:wrap">图5 · 耗时趋势<span id="ds-toggle-duration" style="font-size:10px;padding:2px 7px;border-radius:3px;background:#374151;color:#9ca3af;cursor:pointer">轮次</span><select id="ds-month-duration" style="font-size:11px;padding:2px 6px;border-radius:4px;background:#1e293b;color:#93c5fd;border:1px solid #6366f1;display:none;cursor:pointer;font-family:system-ui,-apple-system,sans-serif"></select></div><div style="position:relative;height:40vh;min-height:280px"><canvas id="ds-chart-duration"></canvas></div><div id="ds-slider-duration-track" class="ds-chart-slider-track" style="margin:0"><div id="ds-slider-duration-thumb" class="ds-chart-slider-thumb"><div class="ds-slider-handle ds-slider-handle-left"></div><div class="ds-slider-handle ds-slider-handle-right"></div></div></div><div id="ds-slider-duration-label" class="ds-chart-slider-label"></div></div>' +
      '<div style="margin-bottom:20px"><div style="font-size:12px;color:#9ca3af;font-weight:500;margin-bottom:8px;display:flex;align-items:center;gap:4px;flex-wrap:wrap">图6 · 速率趋势<span id="ds-toggle-tokenrate" style="font-size:10px;padding:2px 7px;border-radius:3px;background:#374151;color:#9ca3af;cursor:pointer">轮次</span><select id="ds-month-tokenrate" style="font-size:11px;padding:2px 6px;border-radius:4px;background:#1e293b;color:#93c5fd;border:1px solid #6366f1;display:none;cursor:pointer;font-family:system-ui,-apple-system,sans-serif"></select></div><div style="position:relative;height:40vh;min-height:280px"><canvas id="ds-chart-tokenrate"></canvas></div><div id="ds-slider-tokenrate-track" class="ds-chart-slider-track" style="margin:0"><div id="ds-slider-tokenrate-thumb" class="ds-chart-slider-thumb"><div class="ds-slider-handle ds-slider-handle-left"></div><div class="ds-slider-handle ds-slider-handle-right"></div></div></div><div id="ds-slider-tokenrate-label" class="ds-chart-slider-label"></div></div>' +
    '</div>' +
      '<div style="margin-bottom:20px"><div style="font-size:12px;color:#9ca3af;font-weight:500;margin-bottom:8px">图7 · 模型Token占比</div><div style="position:relative;height:40vh;min-height:280px;max-width:500px;margin:0 auto"><canvas id="ds-chart-modeltokens"></canvas></div></div>' +
  ...
  ['token','cost','rate','duration','tokenrate'].forEach(function(k) {
  ...
  ['token','cost','rate','requests','duration','tokenrate'].forEach(function(k) {
...
// ===== 图表实例缓存 =====
var _chartInstances = { token: null, cost: null, rate: null, requests: null, duration: null, tokenrate: null };
// ===== 图表日视图模式开关（false=轮次，true=日期） =====
var _chartDayMode = { token: false, cost: false, rate: false, requests: true, duration: false, tokenrate: false };
// ===== 图表当前显示的月份索引（-1=最新月份） =====
var _chartMonthIdx = { token: -1, cost: -1, rate: -1, requests: -1, duration: -1, tokenrate: -1 };
...
function renderCharts() {
  ...
  ['token','cost','rate','requests','duration','tokenrate','modeltokens'].forEach(function(k){if(_chartInstances[k]){_chartInstances[k].destroy();_chartInstances[k]=null;}});
  // ===== 工具：根据模式选取数据集 =====
    ...
    if (ctx5) {
      ...
      _chartInstances.duration = new Chart(ctx5, {
        ...
        data: {
          ...
          datasets: [
            { label: '平均耗时(s)', data: pick('duration', durationRound, durationDay), backgroundColor: 'rgba(96,165,250,0.7)', borderColor: '#60a5fa', borderWidth: 1, borderRadius: 2 }
          ]
        },
        options: {
          responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false },
  ...
  // ===== 图6 · 速率趋势 =====
  var ctx6 = doc.getElementById('ds-chart-tokenrate');
  if (ctx6) {
    ctx6 = ctx6.getContext('2d');
    if (ctx6) {
      var trUseDay = _chartDayMode.tokenrate;
      ...
      _chartInstances.tokenrate = new Chart(ctx6, {
        type: 'bar',
        data: {
          labels: pick('tokenrate', labels, dayLabels),
          datasets: [
            { label: '平均速率(t/s)', data: pick('tokenrate', trRound, trDay), backgroundColor: 'rgba(251,191,36,0.7)', borderColor: '#fbbf24', borderWidth: 1, borderRadius: 2 }
          ]
        },
        options: {
          responsive: true, maintainAspectRatio: false, interaction: { mode: 'index', intersect: false },
          plugins: {
            legend: { position: 'top', labels: { color: '#9ca3af', font: { family: fontFam }, boxWidth: 12, padding: 12, usePointStyle: true } },
            tooltip: { callbacks: { afterBody: function(items) { var idx = items[0].dataIndex; var d = srcData('tokenrate')[idx]; if (!d) return []; var t = new Date(d.timestamp?d.timestamp:dD[idx]?Date.now():0).toLocaleString('zh-CN'); var p = d.priceType === 'new-peak' ? '🔴 高峰' : d.priceType === 'new-offpeak' ? '🟢 非高峰' : '⚪ 旧价格'; return ['对话时间: ' + t, '时段: ' + p]; } } },
            zoom: { pan: { enabled: true, mode: 'x' }, zoom: { wheel: { enabled: true }, pinch: { enabled: true }, drag: { enabled: true, backgroundColor: 'rgba(99,102,241,0.08)', borderColor: '#6366f1', borderWidth: 1 }, mode: 'x' }, onPanComplete:function(ctx){syncSliderFromChart(ctx.chart)}, onZoomComplete:function(ctx){syncSliderFromChart(ctx.chart)} }
          },
          scales: {
            x: trUseDay?{stacked:false,ticks:{color:'#6b7280',font:{family:fontFam},maxTicksLimit:20},grid:{color:'rgba(55,65,81,0.4)'},title:{display:true,text:'日期',color:'#6b7280',font:{size:11,family:fontFam}}}:{min:zR?zR.l:void 0,max:zR?zR.r:void 0,stacked:false,ticks:{color:'#6b7280',font:{family:fontFam},maxTicksLimit:20},grid:{color:'rgba(55,65,81,0.4)'},title:{display:true,text:'对话轮次',color:'#6b7280',font:{size:11,family:fontFam}}},
            y: { stacked: false, ticks: { color: '#6b7280', font: { family: fontFam } }, grid: { color: 'rgba(55,65,81,0.4)' }, beginAtZero: true, title: { display: true, text: '速率 (t/s)', color: '#6b7280', font: { size: 11, family: fontFam } } }
          }
        }
      });
    }
  ...
  if (pieCtx) {
    ...
    if (pieCtx) {
      ...
      var pieData = pieModels.map(function(m) { return modelTokenMap[m]; });
            ...
            tooltip: {
              callbacks: {
                label: function(ctx) {
                  ...
                  return ctx.label + ': ' + val.toLocaleString() + ' tokens (' + pct + '%)';
                }
              }
            }
          },
  ...
  initChartSliders();
  // 对日期模式的图表应用月份缩放
  var dayKeys = ['token','cost','rate','duration','tokenrate'];
...
function showUsageDetail(model, rawUsage) {
  ...
  if (isMobile) {
    panel.style.cssText = 'position:fixed;top:0;left:0;width:100vw;height:100vh;transform:translateY(100%);z-index:999999;background:#0e1520;font-family:system-ui,-apple-system,sans-serif;color:#e5e7eb;transition:transform 0.2s ease,opacity 0.2s ease;opacity:0;pointer-events:none;box-sizing:border-box;overflow-y:auto';
  } else {
    panel.style.cssText = 'position:fixed;bottom:0;left:50%;transform:translateX(-50%) translateY(100%);z-index:999999;background:#0e1520;border:1px solid #374151;border-radius:12px 12px 0 0;padding:16px;font-family:system-ui,-apple-system,sans-serif;color:#e5e7eb;box-shadow:0 20px 60px rgba(0,0,0,0.5);transition:transform 0.3s ease,opacity 0.3s ease;opacity:0;pointer-events:none;width:min(700px,100%);max-height:80vh;overflow-y:auto;max-width:100vw;box-sizing:border-box';
  }
