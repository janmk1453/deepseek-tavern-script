...
function showUsageDetail(model, rawUsage) {
  ...
  var panel = doc.createElement('div'); panel.id = 'ds-usage-panel'; panel.style.cssText = 'position:fixed;bottom:0;left:50%;transform:translateX(-50%) translateY(100%);z-index:999999;background:#0e1520;border:1px solid #374151;border-radius:12px 12px 0 0;padding:16px;font-family:system-ui,-apple-system,sans-serif;color:#e5e7eb;box-shadow:0 20px 60px rgba(0,0,0,0.5);transition:transform 0.3s ease,opacity 0.3s ease;opacity:0;pointer-events:none;width:min(700px,100%);max-height:80vh;overflow-y:auto;max-width:100vw;box-sizing:border-box';
  panel.innerHTML = '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;padding-bottom:12px;border-bottom:1px solid #374151"><span style="font-weight:600;font-size:14px">使用详情 — ' + model + '</span><div id="ds-usage-close" style="width:28px;height:28px;background:#374151;border-radius:6px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:14px;color:#9ca3af;flex-shrink:0;margin-left:auto">✕</div></div><pre style="background:#060a10;border:1px solid #374151;border-radius:6px;padding:12px;font-size:11px;line-height:1.5;overflow-x:auto;white-space:pre-wrap;word-break:break-all;color:#d1d5db;margin:0">' + JSON.stringify(rawUsage, null, 2) + '</pre>';
  doc.body.appendChild(overlay); doc.body.appendChild(panel);
  var dsUS = doc.createElement('style'); dsUS.textContent = '#ds-usage-panel.ds-open{transform:translateX(-50%) translateY(0)!important;opacity:1!important;pointer-events:auto!important}'; doc.head.appendChild(dsUS);
