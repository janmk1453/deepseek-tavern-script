...
(function() {
  ...
  function shortModelV2(m) { return m; }
  function _ds_esc(t) { return String(t == null ? '' : t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/"/g,'&quot;').replace(/'/g,'&#39;'); }
  function _dsIsUnsafeKey(k) { return k === '__proto__' || k === 'constructor' || k === 'prototype'; }
...
function init() { 
  ...
  // ===== 持久化保存快捷函数 =====
  function _dsPruneHistoryDetails() {
    try {
      Object.keys(state.saves).forEach(function(k) {
        var s = state.saves[k];
        if (!s || !Array.isArray(s.history) || s.history.length <= DETAIL_KEEP) return;
        var hs = s.history.slice().sort(function(a, b) { return b.timestamp - a.timestamp; });
        for (var i = DETAIL_KEEP; i < hs.length; i++) {
          var h = hs[i];
    ...
    } catch(e) { _ds_log.warn('裁剪历史详细数据失败：' + ((e && e.message) || e)); }
  }
  function saveSaves() { _dsPruneHistoryDetails(); saveData(SAVES_STORAGE, JSON.stringify(state.saves)); }
  function getCurrentSaveObj() { if (state.currentSave && state.currentSave !== '__all__') return state.saves[state.currentSave] || null; return null; }
  // 自定义余额跟随存档：具体存档用各自存储的余额，未设置时回退全局存储值；合并统计（__all__）直接使用全局存储值
  function syncCustomBalanceFromSave() {
    var s = getCurrentSaveObj();
    if (s && s.customBalance !== undefined && s.customBalance !== null && s.customBalance !== '') {
      state.customBalance = s.customBalance;
    } else {
      var g = loadData(CUSTOM_BALANCE_STORAGE);
      state.customBalance = (g !== null && g !== '') ? g : null;
    }
  }
  function persistCustomBalance() {
    var s = getCurrentSaveObj();
    if (s) {
      s.customBalance = (state.customBalance !== null && state.customBalance !== '') ? String(state.customBalance) : null;
      saveSaves();
    } else {
  ...
  function exportHistory() {
    ...
    var exportSaves = JSON.parse(JSON.stringify(state.saves || {}));
    Object.keys(exportSaves).forEach(function(k) {
      var sv = exportSaves[k];
      if (sv && Array.isArray(sv.history)) {
        sv.history.forEach(function(h) {
          if (h && typeof h === 'object') {
            delete h.messages;
            delete h.fullRequest;
            delete h.fullResponse;
    ...
    try { toastr.success('已导出 ' + Object.keys(state.saves).length + ' 个存档（不含 API 密钥与聊天内容）'); } catch(e) {}
  }
  ...
  function normalizeImportData(raw) {
    ...
    Object.keys(d.saves).forEach(function(k) {
      if (_dsIsUnsafeKey(k)) return;
      var s = d.saves[k];
      if (!s || typeof s !== 'object') { skipped.saves++; return; }
      var ns = { name: s.name || k, character: (s.character === undefined ? '' : s.character), customBalance: (s.customBalance === undefined ? null : s.customBalance) };
      ...
      if (Array.isArray(s.history)) {
        s.history.forEach(function(h) {
          ...
          Object.keys(h).forEach(function(f) { if (_dsIsUnsafeKey(f)) return; if (nh[f] === undefined) nh[f] = h[f]; });
          hs.push(nh);
        });
      }
      ns.history = hs;
      Object.keys(s).forEach(function(f) { if (_dsIsUnsafeKey(f) || f === 'history') return; if (ns[f] === undefined) ns[f] = s[f]; });
      saves[k] = ns;
    });
    d.saves = saves;
  ...
  function applyImportedData(d, mode) {
    ...
    Object.keys(importedSaves).forEach(function(k) { if (_dsIsUnsafeKey(k)) return;
  ...
  function generateDebugBatch() {
    ...
    for (var d = 0; d < totalDays && generated < count; d++) {
      ...
      for (var i = 0; i < entriesPerDay && generated < count; i++) {
        ...
        var tRate = (dur - ttft) > 50 ? Math.round(o / (dur - ttft) * 1000) : 0;
        var promptTotal = h + m;
...
function modelEditorRowHtml(model, entry, isBuiltin) {
  ...
  var modelInput = '<input type="text" class="ds-cm-model" value="' + _ds_esc(model) + '" placeholder="模型名" ' + (isBuiltin ? 'readonly style="' + roStyle + '"' : 'style="' + inpStyle + '"') + '>';
  ...
  return '<div class="ds-cm-row" data-model="' + _ds_esc(model) + '" data-builtin="' + (isBuiltin ? '1' : '0') + '" style="' + rowStyle + '">' +
...
function buildChartModelSelectHtml() {
  ...
  var html = '<option value="__all__">全部模型</option>' + models.map(function(m) { return '<option value="' + _ds_esc(m) + '">' + _ds_esc(shortModelV2(m)) + '</option>'; }).join('');
  return html;
}
...
function renderCharts() {
  ...
  if (pieCtx) {
    ...
    if (pieCtx) {
      ...
      var pieLabels = pieModels.map(function(m) { return _ds_esc(shortModel(m)); });
...
var _ds_last_toggle = 0;                          // 防抖时间戳
  ...
  function openComparePanel(tsBefore, tsAfter) {
    ...
    body.innerHTML = '<div class="ds-compare-info"><span><span class="ds-info-old">旧请求</span> #' + (s.history.indexOf(a) + 1) + ' \u00b7 ' + new Date(a.timestamp).toLocaleString('zh-CN', {month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}) + ' \u00b7 ' + _ds_esc(a.model) + '</span><span style="color:#6b7280">\u2192</span><span><span class="ds-info-new">新请求</span> #' + (s.history.indexOf(b) + 1) + ' \u00b7 ' + new Date(b.timestamp).toLocaleString('zh-CN', {month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit'}) + ' \u00b7 ' + _ds_esc(b.model) + '</span></div>';
...
function showUsageDetail(model, rawUsage) {
  ...
  var contentHtml = '<div style="' + headerStyle + '"><span style="font-weight:600;font-size:14px">使用详情 — ' + _ds_esc(model) + '</span><div id="ds-usage-close" style="width:28px;height:28px;background:#374151;border-radius:6px;display:flex;align-items:center;justify-content:center;cursor:pointer;font-size:14px;color:#9ca3af;flex-shrink:0;margin-left:auto">✕</div></div><div style="' + bodyStyle + '">' + renderContent() + '</div>';
