...
var _ds_last_toggle = 0;                          // 防抖时间戳
  ...
  }function refreshUI() { var p = window.parent || window; var doc = p.document; var el = function(id) { return doc.getElementById(id); }; refreshSaveSelect(); refreshOverviewModelSelect(); var s = getSelectedSave(); if (!s) return; var ltc=0,lic=0,loc=0,lr=0; (s.history||[]).forEach(function(h){var u={timestamp:h.timestamp,model:h.model,prompt_cache_hit_tokens:h.cache_hit_tokens||0,prompt_cache_miss_tokens:h.cache_miss_tokens||0,completion_tokens:h.completion_tokens||0};var c=calcCost(u);h.cost=c.total;h.input_cost=c.input;h.output_cost=c.output;h.priceType=c.priceType;ltc+=c.total;lic+=c.input;loc+=c.output;if(isDeepSeekOfficialModel(h.model)){lr++}}); s.total_cost=ltc;s.input_cost=lic;s.output_cost=loc;s.rounds=lr; var om=state.overviewModel||'__all__'; var ovH=(s.history||[]).filter(function(h){return om==='__all__'||h.model===om;}); var ov={total_tokens:0,total_cost:0,input_tokens:0,output_tokens:0,cache_hit_tokens:0,cache_miss_tokens:0,input_cost:0,output_cost:0,rounds:0}; ovH.forEach(function(h){ov.total_tokens+=h.total_tokens||0;ov.total_cost+=h.cost||0;ov.input_tokens+=h.prompt_tokens||0;ov.output_tokens+=h.completion_tokens||0;ov.cache_hit_tokens+=h.cache_hit_tokens||0;ov.cache_miss_tokens+=h.cache_miss_tokens||0;ov.input_cost+=h.input_cost||0;ov.output_cost+=h.output_cost||0;if(isDeepSeekOfficialModel(h.model)){ov.rounds++}}); var _noPrice = ovH.some(function(h){ return !hasPriceForModel(h.model); }); if (el('ds-save-time')) el('ds-save-time').textContent = state.currentSave === '__all__' ? '' : '开始于 ' + formatStartTime(s.startTime); if (el('ds-total-tokens')) el('ds-total-tokens').textContent = (ov.total_tokens || 0).toLocaleString(); if (el('ds-total-cost')) el('ds-total-cost').textContent = _noPrice ? '¥价格未设置' : '\u00A5' + (ov.total_cost || 0).toFixed(4); if (el('ds-history-count')) el('ds-history-count').textContent = '当前' + (ovH.length || 0) + '条'; if (el('ds-total-cache-hit')) el('ds-total-cache-hit').textContent = (ov.cache_hit_tokens || 0).toLocaleString(); if (el('ds-total-cache-miss')) el('ds-total-cache-miss').textContent = (ov.cache_miss_tokens || 0).toLocaleString(); if (el('ds-total-output')) el('ds-total-output').textContent = (ov.output_tokens || 0).toLocaleString(); if (el('ds-rounds')) el('ds-rounds').textContent = '基于 ' + (ov.rounds || 0) + ' 轮'; var tp = 0, th = 0; ovH.forEach(function(i) { tp += i.prompt_tokens || 0; th += i.cache_hit_tokens || 0; }); if (el('ds-weighted-rate')) el('ds-weighted-rate').textContent = (tp > 0 ? (th / tp * 100) : 0).toFixed(1) + '%'; if ((ov.rounds || 0) > 0) { if (el('ds-avg-tokens')) el('ds-avg-tokens').textContent = Math.round((ov.total_tokens || 0) / ov.rounds) + ' tokens'; var _durSum=0,_durCnt=0,_trSum=0,_trCnt=0;ovH.forEach(function(h){if(h.duration){_durSum+=h.duration;_durCnt++}if(h.tokenRate){_trSum+=h.tokenRate;_trCnt++}});if(el('ds-avg-duration'))el('ds-avg-duration').textContent=_durCnt>0?(_durSum/_durCnt/1000).toFixed(1)+'s':'--';if(el('ds-avg-tokenrate'))el('ds-avg-tokenrate').textContent=_trCnt>0?Math.round(_trSum/_trCnt)+' t/s':'--';if(el('ds-dur-dot')){if(_durCnt>0){var _ds=_durSum/_durCnt/1000;el('ds-dur-dot').style.background=_ds<30?'#34d399':_ds<60?'#fbbf24':'#f87171'}else{el('ds-dur-dot').style.background='#6b7280'}}if(el('ds-rate-dot')){if(_trCnt>0){var _tr=_trSum/_trCnt;el('ds-rate-dot').style.background=_tr>50?'#34d399':_tr>20?'#fbbf24':'#f87171'}else{el('ds-rate-dot').style.background='#6b7280'}}if(el('ds-cost-dot')){var _ac=parseFloat(el('ds-avg-cost').textContent.replace('¥',''))||0;el('ds-cost-dot').style.background=_noPrice?'#6b7280':(_ac<0.005?'#34d399':_ac<0.02?'#fbbf24':'#f87171')}if(el('ds-tokens-dot')){var _at=parseInt(el('ds-avg-tokens').textContent.replace(/[^0-9]/g,''))||0;el('ds-tokens-dot').style.background=_at<1000?'#34d399':_at<3000?'#fbbf24':'#f87171'}if (el('ds-avg-cost')) el('ds-avg-cost').textContent = _noPrice ? '¥价格未设置' : '\u00A5' + ((ov.total_cost || 0) / ov.rounds).toFixed(4); } var sv = (ov.cache_hit_tokens || 0) * 0.98 / 1e6; if (el('ds-savings')) el('ds-savings').textContent = '\u00A5' + sv.toFixed(4); if (el('ds-savings-tokens')) el('ds-savings-tokens').textContent = (ov.cache_hit_tokens || 0).toLocaleString(); if (el('ds-input-cost')) el('ds-input-cost').textContent = _noPrice ? '¥价格未设置' : '\u00A5' + (ov.input_cost || 0).toFixed(4); if (el('ds-input-tokens')) el('ds-input-tokens').textContent = (ov.input_tokens || 0).toLocaleString(); if (el('ds-output-cost')) el('ds-output-cost').textContent = _noPrice ? '¥价格未设置' : '\u00A5' + (ov.output_cost || 0).toFixed(4); if (el('ds-output-tokens')) el('ds-output-tokens').textContent = (ov.output_tokens || 0).toLocaleString(); var be = el('ds-balance'); if (be) { var balText = '\u00A5'; if (state.customBalance !== null && state.customBalance !== '') { balText += parseFloat(state.customBalance).toFixed(2) + ' CNY'; } else if (state.balance && state.balance.balance) { balText += parseFloat(state.balance.balance).toFixed(2) + ' ' + state.balance.currency; } else { balText += '0.00 CNY'; } be.textContent = balText; } var rem = el('ds-balance-remaining'); if (rem) { var r = calculateRemainingRounds({ rounds: ov.rounds, total_cost: ov.total_cost, history: ovH }); rem.textContent = r !== null ? '预计还可进行 ' + r + ' 轮对话' : ''; } var PAGE_SIZE = 20; var totalPages = Math.ceil((ovH.length || 0) / PAGE_SIZE); if (_historyPage >= totalPages) _historyPage = Math.max(0, totalPages - 1); if (_historyPage < 0) _historyPage = 0; var startIdx = _historyPage * PAGE_SIZE; var pageItems = ovH.slice(startIdx, startIdx + PAGE_SIZE); if (ovH && ovH.length > 0 && el('ds-history')) { el('ds-history').innerHTML = pageItems.map(function(i, idx) {
...
function showUsageDetail(model, rawUsage) {
  ...
  function renderContent() {
    ...
    var costInfo = '<div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;margin-bottom:8px;font-size:11px">' +
      ...
      '<div style="padding:8px;background:#060a10;border:1px solid #374151;border-radius:6px"><div style="color:#6b7280">输入费用</div><div style="color:#fbbf24">' + fmtCost(model, rawUsage.input_cost || 0, 6) + '</div></div>' +
      '<div style="padding:8px;background:#060a10;border:1px solid #374151;border-radius:6px"><div style="color:#6b7280">输出费用</div><div style="color:#fbbf24">' + fmtCost(model, rawUsage.output_cost || 0, 6) + '</div></div>' +
      '<div style="padding:8px;background:#060a10;border:1px solid #374151;border-radius:6px;grid-column:1/-1"><div style="color:#6b7280">总费用</div><div style="color:#fbbf24;font-weight:700;font-size:13px">' + fmtCost(model, rawUsage.cost || 0, 6) + '</div></div>' +
      '</div>';
