...
function init() { 
  ...
  function loadSavedData() { 
    try { 
      ...
      if (state.settings.newPricingDate === undefined) state.settings.newPricingDate = new Date('2026-08-17T00:00:00+08:00').getTime(); if (state.settings.customModels === undefined) state.settings.customModels = []; if (state.settings.peakHours === undefined) state.settings.peakHours = JSON.parse(JSON.stringify(DEFAULT_PEAK_HOURS));
  ...
  function normalizeImportData(raw) {
    ...
    if (settings.newPricingDate === undefined) settings.newPricingDate = new Date('2026-08-17T00:00:00+08:00').getTime(); if (settings.customModels === undefined) settings.customModels = []; if (settings.peakHours === undefined) settings.peakHours = JSON.parse(JSON.stringify(DEFAULT_PEAK_HOURS));
    d.settings = settings;
